...
# Глобальный перехватчик для поддержки DRY_RUN
# Использование: _exec "описание действия" команда аргументы
_exec() {
    local desc="${1:-}"
    shift # Удаляем описание из аргументов, оставляя только саму команду
    local cmd=("$@")

    # Проверяем режим DRY_RUN
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        log_warn "[DRY-RUN] Пропущено: $desc -> (${cmd[*]})"
        return 0
    fi

    # Обычный боевой режим выполнения
    log_debug "Выполнение: $desc -> (${cmd[*]})"
    "${cmd[@]}"
}
